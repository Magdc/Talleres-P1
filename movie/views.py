...
def statistics_view(request):
    ...
    prueba = []
    for genre in genres:
        prueba.append(genre.split(',')[0].strip())
    actualgenres = {}
    for genre in prueba:
        if genre:
            if genre in actualgenres:
                actualgenres[genre] = actualgenres[genre]+1
            else:
                actualgenres[genre] = 0
        else:
            if "None" in actualgenres:
                actualgenres["None"] = actualgenres["None"]+1
            else:
                actualgenres["None"] = 0
    ...
    bar_positions = range(len(actualgenres))  # Posiciones de las barras

    # Crear la gráfica de barras
    plt.bar(bar_positions, actualgenres.values(), width=bar_width, align='center')

    # Personalizar la gráfica
    plt.title('Movies per genre')
    plt.xlabel('Genre')
    plt.ylabel('Number of genres')
    plt.xticks(bar_positions, actualgenres.keys(), rotation=90)
    plt.subplots_adjust(bottom=0.3)  
    ...
    return render(request, 'statistics.html', {'graphic': graphic,'graphic_per_genre':graphic_per_genre })
